...
class Dbclear extends CI_Controller
{
	...
	public function index(){
        ...
        $excludes = ['buku_induk', 'api_setting', 'api_token', 'bulan', 'hari', 'setting',
            'cbt_jenis', 'cbt_ruang', 'cbt_sesi', 'cbt_token',
            'level_guru', 'level_kelas',
            //'master_mapel', 'master_ekstra', 'master_jurusan',
            'master_tp', 'master_smt', 'master_hari_efektif',
            'users', 'groups', 'users_groups', 'login_attempts', 'users_profile',
            'rapor_admin_setting', 'running_text'];
        ...
        foreach ($tables as $table) {
            if (isset($json[$table])) {
                if (!in_array($table, $excludes)) {
                    //$name = str_replace("cbt_", "", $table);
                    //$name = str_replace("master_", "", $name);
                    //$name = str_replace("kelas_", "", $name);
    ...
    public function hapusTable() {
        ...
        $prefs = [
            ...
            'filename' => $table.'.sql',    // File name - NEEDED ONLY WITH ZIP FILES
