            ...
            <div class="card card-default my-shadow mb-4">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        <button class="btn btn-sm btn-default" onclick="reload()">
                ...
                <div class="card-body">
                    ...
                    <div id="table-info" class="table-responsive d-none">
                        <table id="atas" class="table table-sm table-borderless">
                            <tbody>
                            <tr>
                                <th style="width: 80px; min-width: 80px">Mapel</th>
                                <th style="width: 250px; min-width: 250px" colspan="2" id="info-mapel">Sejarah</th>
                                <th style="width: 80px; min-width: 80px" id="label-jenis">MATERI</th>
                                <th style="width: 250px; min-width: 250px" colspan="2" id="info-judul">MATERI</th>
                            </tr>
                            <tr>
                                <td class="text-bold">Guru</td>
                                <td colspan="2" id="info-guru">....</td>
                                <td class="text-bold">Hari/Tgl.</td>
                                <td colspan="2" id="info-hari">....</td>
                            </tr>
                            <tr>
                                <td class="text-bold">Kelas</td>
                                <td colspan="2" id="info-kelas">....</td>
                                <td class="text-bold">Waktu</td>
                                <td colspan="2" id="info-waktu">....</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div id="preview" class="table-responsive">
                        ...
                        <table id="log" class="table table-sm table-striped table-bordered table-hover"
            ...
            <div class="modal-body">
                <div class="container-fluid border">
                    ...
                    <div id="konten-nilai" class="row p-2">
                        ...
                        <div class="offset-1 col-8 mb-3">
                            <span><b>Info</b></span>
                            <p>
                                Beri nilai nol (<b>0</b>) jika siswa harus mengulang materi/tugas.
                            </p>
                        </div>
                        <div class="col-12">
                        ...
                        <div class="col-12 my-3">
                            <button type="submit" class="btn btn-primary float-right">Simpan Nilai</button>
                        </div>
                    </div>
                    <?= form_close(); ?>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        dropMapel.on('change', function () {
            ...
            currentURL = base_url + "elearning/getmaterimapel?id=" + $(this).val() + '&id_guru='+idGuru;
            $.ajax({
                method: "GET",
                url: currentURL,
        ...
        $('#daftarModal').on('show.bs.modal', function (e) {
            ...
            if (val.selesai == null) {
                html = '<div class="col-12 mt-3">' +
                    ...
                    'Siswa belum menyelesaikan materi.<br /> Jika diberi nilai maka materi/tugas tidak bisa dikerjakan oleh siswa' +
                    '</div></div>';
                konten.append(html);
                $('#method').val('add');
        ...
        $('#formnilai').submit('click', function (e) {
            ...
            $.ajax({
                url: base_url + "elearning/savenilai",
                type: "POST",
                dataType: "JSON",
    ...
    function getLogSiswa() {
        ...
        setTimeout(function () {
            $.ajax({
                type: 'POST',
                url: base_url + 'elearning/loadstatus',
                data: form.serialize() + '&id_kjm=' + selMateri + '&id_kelas=' + selKelas + '&label=' + label,
                success: function (data) {
                    console.log('result', data);
                    ...
                    const tglView = new Date(data.detail.waktu.tgl).toLocaleDateString('id-ID',{
                        weekday: 'long',
                        year: "numeric",
                        month: "long",
                        day: "2-digit",
                    })
                    $('#info-hari').text(': '+ tglView)
                    $('#info-waktu').text(': '+data.detail.waktu.dari + ' ~ ' + data.detail.waktu.sampai)
                    ...
                    var html = '<thead>' +
                        ...
                        '<th rowspan="2" style="min-width: 220px; width: 220px" class="align-middle text-center center" '+ styleHead +'>Nama Siswa</th>' +
                        ...
                        '</tr>' +
                        '<tr style="background-color:lightgrey;">' +
                        '<th style="min-width: 70px; width: 70px" class="text-center align-middle center" '+ styleHead +'>Buka ' + label + '</th>' +
                        '<th style="min-width: 70px; width: 70px" class="text-center align-middle center" '+ styleHead +'>Selesai</th>' +
                    ...
                    $.each(resultAll, function (key, value) {
                        ...
                        let selesai = '- -  :  - -';
                        let nilai = '';
                        let ketMulai = 'Belum memulai';

                        if (value.login != null) {
                            login = createTime(value.login);
                        }

                        console.log('mulai', value.mulai)
                        if (value.mulai != null) {
                            mulai = buatTanggal(value.mulai);
                            if (value.selesai == null) ketMulai = '<i class="fa fa-spinner fa-spin mr-2"></i> Mengerjakan';
                        }

                        const forceNilai = value.mulai != null && value.selesai == null && value.nilai != null;
                        nilai = value.nilai || '';
                        if (value.nilai && value.nilai < 1) {
                            selesai = '- - : - -';
                            ketMulai = 'diulang';
                            mulai = '- - : - -';
                        } else if (forceNilai) {
                            selesai = '';
                            ketMulai = 'Tidak selesai';
                        } else if (value.mulai != null && value.selesai != null) {
