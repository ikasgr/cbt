...
<script>
    const kelas = JSON.parse('<?= json_encode($kelas)?>');
    const arrKelas = [];
    let form;
    let bln = '';
    let thn = '';
    ...
    const styleBorder = ' style="border: 1px solid #8d8d8d; border-collapse: collapse;';
    const styleMinggu = ' style="border: 1px solid #8d8d8d; text-align: center; vertical-align: middle;background-color:#FF9393;';
    const styleHadir = ' style="border: 1px solid #8d8d8d; text-align: center; vertical-align: middle;background-color:#78E96B;';
    const styleTelat = ' style="border: 1px solid #8d8d8d; text-align: center; vertical-align: middle;background-color:#FFF493;';
    const styleAlpha = ' style="border: 1px solid #8d8d8d; text-align: center; vertical-align: middle;background-color:#FFCCCD;';
    const styleCenterMiddle = ' style="border: 1px solid #8d8d8d; text-align: center; vertical-align: middle;';
    const styleLeftMiddle = ' style="border: 1px solid #8d8d8d; vertical-align: middle;';
    const styleKosong = ' style="border: 1px solid #8d8d8d;background-color: #a6a6a6;';

    // style excel
    const styleHead = ' data-fill-color="d3d3d3" data-t="s" data-a-v="middle" data-a-h="center" data-b-a-s="thin" data-f-bold="true" data-a-wrap="true"';
    const styleHeadMinggu = ' data-fill-color="FF9393" data-t="s" data-a-v="middle" data-a-h="center" data-b-a-s="thin" data-f-bold="true" data-a-wrap="true"';
    const styleNormal = ' data-fill-color="ffffff" data-t="s" data-a-v="middle" data-a-h="center" data-b-a-s="thin" data-f-bold="false"';
    const styleNormalMinggu = ' data-fill-color="FF9393" data-t="s" data-a-v="middle" data-a-h="center" data-b-a-s="thin" data-f-bold="false"';
    const style_Hadir = ' data-fill-color="78E96B" data-t="s" data-a-v="middle" data-a-h="center" data-b-a-s="thin" data-f-bold="false"';
    const style_Telat = ' data-fill-color="FFF493" data-t="s" data-a-v="middle" data-a-h="center" data-b-a-s="thin" data-f-bold="false"';
    const style_Alpha = ' data-fill-color="FFCCCD" data-t="s" data-a-v="middle" data-a-h="center" data-b-a-s="thin" data-f-bold="false"';
    const styleEmpty = ' data-fill-color="a6a6a6" data-t="s" data-a-v="middle" data-a-h="center" data-b-a-s="thin" data-f-bold="false"';
    const styleNama = ' data-fill-color="ffffff" data-t="s" data-a-v="middle" data-b-a-s="thin" data-f-bold="false"';
    ...
    function createTable(data) {
        ...
        table = '<div id="jdl" style="width:100%;"><p style="text-align:center;font-size:14pt; font-weight: bold">KEHADIRAN BULANAN SISWA</p></div>' +
            ...
            '<tr>' +
            '<th rowspan="3" ' + styleBorder + ' min-width: 40px;"' + styleHead + '>No</th>' +
            '<th rowspan="3" ' + styleBorder + ' min-width: 80px;"' + styleHead + '>N I S</th>' +
            '<th rowspan="3" ' + styleBorder + '"' + styleHead + '>Nama</th>' +
            '<th rowspan="3" ' + styleCenterMiddle + '"' + styleHead + '>Kelas</th>' +
        ...
        for (let i = 0; i < jmlHari; i++) {
            ...
            if (hari === 'Min') {
                table += '<th rowspan="2" class="tanggal" ' + styleMinggu + '" ' + styleHeadMinggu + '>'+ hari + ' ' + '<br>' + tg + '</th>';
            } else {
                if (data.mapels[tg] != null) {
                    table += '<th colspan="2" class="tanggal" ' + styleCenterMiddle + '"' + styleHead + '>'+ hari + ' ' + '<br>' + tg + '</th>';
                }
            }
        }

        table += '<th colspan="3" ' + styleCenterMiddle + '"' + styleHead + '>Materi</th>' +
            '<th colspan="3" ' + styleCenterMiddle + '"' + styleHead + '>Tugas</th>' +
            '</tr>';
        ...
        $.each(data.log, function (key, value) {
            ...
            for (let i = 0; i < jmlHari; i++) {
                ...
                const infoJadwal = data.info.find(function (jdwl) {
                    return jdwl.id_hari == d.getDay() && jdwl.id_mapel
                })
                ...
                if (hari === 'Min') {
                    ...
                    if (adaJadwal) {
                        var tglJadwal = thn +'-'+(Number(bln) < 10 ? '0' + bln : bln)+'-'+tglm
                        var adaMateri = data.materi[tglJadwal] != null && data.materi[tglJadwal][1] != null;
                        if (adaMateri) {
                            console.log('mtr', data.materi[tglJadwal][1])
                            var logMateri = value.materi[i] || [];
                            if (logMateri.length > 0) {
                                let jamMateri = logMateri[0]?.jam;
                                if (logMateri.length > 1) {
                                    logMateri.forEach(item => {
                                        if (jamMateri === undefined || item.jam > jamMateri) {
                                            jamMateri = item.jam;
                                        }
                                    });
                                }
                                ...
                                if (jamMateri) {
                                    if (terlambat(value.materi[i][0], infoJadwal)) {
                        ...
                        var adaTugas = data.materi[tglJadwal] != null && data.materi[tglJadwal][2] != null;
                        if (adaTugas) {
                            console.log('tgs', data.materi[tglJadwal][2])
                            var logTugas = value.tugas[i] || [];
                            if (logTugas.length > 0) {
                                let jamTugas = logTugas[0]?.jam;
                                if (logTugas.length > 1) {
                                    logTugas.forEach(item => {
                                        if (jamTugas === undefined || item.jam > jamTugas) {
                                            jamTugas = item.jam;
                                        }
                                    });
                                }
                                ...
                                if (jamTugas) {
                                    if (terlambat(value.tugas[i][0], infoJadwal)) {
                                        table += '<td ' + styleTelat + '"'+ style_Telat +'>&check;</td>';
                                        tlTugas++;
                                    } else {
                                        table += '<td ' + styleHadir + '"'+ style_Hadir +'>&check;</td>';
    ...
    $(document).ready(function () {
        ...
        function reload(mapel, kls, bln, thn, force) {
            ...
            if (!empty && f) {
                ...
                setTimeout(function () {
                    $.ajax({
                        url: base_url + 'elearning/loadabsensimapel',
                        type: "POST",
                        dataType: "json",
        ...
        selBulan.select2({width: '100%', theme: 'bootstrap4'});
        selTahun.select2({width: '100%', theme: 'bootstrap4'});
    });

    function terlambat(value, jadwal) {
        if (jadwal) {
            var tglJadwal = value.jadwal_materi;
            var mulaiKbm = jadwal.sampai;
            var dateMulai = new Date(tglJadwal + "T" + mulaiKbm);
            var logMulai = new Date(value.log_time);
            //console.log('jad', dateMulai.getTime())
            //console.log('sis', logMulai.getTime())
            return logMulai.getTime() > dateMulai.getTime()
        } else {
            return false
        }
    }
