...
<script>
    ...
    function createTable(data) {
        ...
        $.each(data.bulans, function (k, v) {
            ...
            konten += '<th rowspan="2" class="tanggal" style="border: 1px solid #c0c0c0; text-align: center; vertical-align: middle;margin: 0px;" ' + styleHead + '>NR</th>';
        });
        ...
        $.each(data.bulans, function (i, bln) {
            ...
            $.each(sortedMateri, function (key, tgl) {
                console.log('sort', key, tgl)
                var d2 = new Date(data.tahun  + '-' + bln + '-' + tgl);
                var hari = weekday[d2.getDay()];
                console.log('log', tgl, bln, data.tahun)
                konten += '<th class="tanggal" style="border: 1px solid #c0c0c0; text-align: center; vertical-align: middle;margin: 0px;" ' + styleHead + '>' + hari + '<br/>'+tgl+'</th>';
                no++;
                colWidth += ',4';
            });
        });
    ...
    $(document).ready(function () {
        ...
        function reload(mapel, kls, thn, smt) {
            ...
            if (!empty) {
                ...
                setTimeout(function () {
                    $.ajax({
                        url: base_url + 'elearning/loadnilaimapel?' + newData,
                        type: "GET",
