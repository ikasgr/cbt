        ...
        <div class="container-fluid">
            <div class="card my-shadow mb-4">
                <div class="card-header py-3">
                   ...
                   <button type="button" id="download-excel" class="btn btn-sm btn-info float-right" data-toggle="tooltip" title="Download Excel Nilai Akhir">
                        <i class="fa fa-file-excel ml-1 mr-1"></i><span class="d-none d-sm-inline-block ml-1">Ekspor ke Excel</span>
                    </button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.each(arrSiswa, function (i, v) {
            ...
            // Ambil nilai RPH, PTS, dan PAS
            var nhar = parseFloat(nilai.nhar) || 0;
            var npts = parseFloat(nilai.npts) || 0;
            var npas = parseFloat(nilai.npas) || 0;
    
            // Hitung nilai AKHIR
            var nilaiAkhir = Math.round(((nhar * bobotPH) / 100) + ((npts * bobotPTS) / 100) + ((npas * bobotPAS) / 100));
            if (isNaN(nilaiAkhir)) nilaiAkhir = ''; // Jika ada error (mirip IFERROR)
    
            // Hitung PRED berdasarkan nilai AKHIR
            var predikat = '';
            if (nilaiAkhir > pre_bsd) {
                predikat = 'A';
            } else if (nilaiAkhir > pre_csd) {
                predikat = 'B';
            } else if (nilaiAkhir > pre_dsd) {
                predikat = 'C';
            } else if (nilaiAkhir < pre_c) {
                predikat = 'D';
            }
            ...
            dataSiswa.push(
                [
                    ...
                    nilaiAkhir, // Gunakan nilai yang sudah dihitung
                    predikat,   // Gunakan predikat yang sudah dihitung
                    nilai.p_deskripsi, nilai.k_rata_rata, nilai.k_predikat, nilai.k_deskripsi
                ]
            );
            row++;
        });
...
$("#download-excel").click(async function (event) {
    // Ensure namaMapel and namaKelas are defined
    const namaMapel = window.namaMapel || 'Unknown Subject';
    const namaKelas = window.namaKelas || 'Unknown Class';

    // Retrieve table data
    const table = $('#t-siswa').jexcel('getData');

    // Define headers
    const customHeaders = [
        'No.', 'NISN', 'Nama Siswa', 'Rata-rata Harian', 'Nilai PTS', 'Nilai PAS',
        'Nilai Akhir', 'Predikat', 'Deskripsi', 'Nilai', 'Predikat', 'Deskripsi'
    ];
    ...
    if (table.length > 0) {
        try {
            ...
            // Set page setup for F4 paper size (210 mm × 330 mm)
            worksheet.pageSetup = {
                paperSize: 9,
            ...
            // Add title
            worksheet.addRow(['LAPORAN NILAI AKHIR SISWA']);
            worksheet.mergeCells(`A1:${String.fromCharCode(65 + customHeaders.length - 1)}1`);
            worksheet.getRow(1).font = { bold: true, size: 16 };
            worksheet.getRow(1).alignment = { horizontal: 'center' };

            // Add subject and class info
            worksheet.addRow([`Mata Pelajaran: ${namaMapel}`]);
            worksheet.mergeCells(`A2:${String.fromCharCode(65 + customHeaders.length - 1)}2`);
            worksheet.getRow(2).font = { bold: true, size: 12 };
            worksheet.getRow(2).alignment = { horizontal: 'center' };

            worksheet.addRow([`Kelas: ${namaKelas}`]);
            worksheet.mergeCells(`A3:${String.fromCharCode(65 + customHeaders.length - 1)}3`);
            ...
            // Add nested headers with grey background
            worksheet.addRow(['', 'DATA SISWA', '', 'NILAI PENGETAHUAN', '', '', '', '', '', 'NILAI KETERAMPILAN', '', '']);
            worksheet.mergeCells('B5:C5');
            worksheet.mergeCells('D5:I5');
            worksheet.mergeCells('J5:L5');
            worksheet.getRow(5).font = { bold: true };
            worksheet.getRow(5).alignment = { horizontal: 'center', wrapText: true };
            ...
            // Add main headers with grey background
            worksheet.addRow(customHeaders);
            worksheet.getRow(6).font = { bold: true };
            worksheet.getRow(6).alignment = { horizontal: 'center', wrapText: true };
            ...
            // Add data with row numbering, color, and bold for Nilai Akhir
            table.forEach((row, index) => {
                const rowWithNoUrut = [(index + 1), ...row];
                const excelRow = worksheet.addRow(rowWithNoUrut);
                // Apply background color and bold to Nilai Akhir column (column G, index 7)
                excelRow.getCell(7).fill = {
                    type: 'pattern',
                    pattern: 'solid',
                    fgColor: { argb: 'B3E5FC' } // Light blue
                };
                excelRow.getCell(7).font = { bold: true }; // Bold formatting
                // Ensure all cells in data rows are centered and wrap text
            ...
            worksheet.columns = [
                { width: 4, alignment: { wrapText: true } },
                { width: 12, alignment: { horizontal: 'center', wrapText: true } },
                { width: 25, alignment: { horizontal: 'center', wrapText: true } },
                { width: 5, alignment: { horizontal: 'center', wrapText: true } },
                { width: 5, alignment: { horizontal: 'center', wrapText: true } },
                { width: 5, alignment: { horizontal: 'center', wrapText: true } },
                { width: 5, alignment: { horizontal: 'center', wrapText: true } },
                { width: 5, alignment: { horizontal: 'center', wrapText: true } },
                { width: 40, alignment: { wrapText: true } },
                { width: 5, alignment: { horizontal: 'center', wrapText: true } },
                { width: 5, alignment: { horizontal: 'center', wrapText: true } },
                { width: 40, alignment: { wrapText: true } },
                { width: 0 }
            ...
            saveAs(blob, `Nilai_Akhir_${namaMapel}_${namaKelas}.xlsx`);
        } catch (error) {
            Swal.fire({
                title: "ERROR",
                text: "Gagal menghasilkan file Excel",
                icon: "error"
            });
        }
    } else {
