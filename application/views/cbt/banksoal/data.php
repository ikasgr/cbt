...
$ftw = isset($pecah[1]) ? "{$pecah[0]} {$pecah[1]}" : $pecah[0];
$timeshow = date('d-m-Y', strtotime($ftw));
$timeshow2 = date("H:i", strtotime($ftw));
$nselesai = date("Y-m-d H:i:s", strtotime($ftw));
$nsekarang = date("Y-m-d H:i:s");

$waktu_selesai = strtotime($nselesai);
$waktu_sekarang = strtotime($nsekarang);
$diff = max(0, $waktu_selesai - $waktu_sekarang); // Menggunakan max untuk memastikan tidak negatif

$jam = floor($diff / 3600); // 3600 detik = 1 jam
$menit = floor(($diff % 3600) / 60);
$diff2 = $waktu_sekarang - $waktu_selesai;

$dnone = (!$this->ion_auth->is_admin() && $nselesai < $nsekarang) ? 'd-none' : '';
?>
            ...
            <div class="card card-default my-shadow mb-4">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        <a href="<?= base_url('cbtbanksoal?id=' . urlencode($id_guru) . '&mode=' . urlencode($mode)) ?>" 
                           class="btn btn-sm btn-default">
                            <i class="fa fa-sync"></i> 
                            <span class="d-none d-sm-inline-block ml-1">Reload</span>
                        </a>
                        <a href="<?= base_url('cbtbanksoal/addBank') ?>" 
                           class="btn btn-primary btn-sm ml-1 <?= $dnone ?>">
                            <i class="fas fa-plus-circle"></i> Tambah Bank Soal
                        </a>
                        <button type="button" 
                                data-toggle="modal" 
                                data-target="#openAllBankSoal"
                                class="btn btn-sm btn-success <?= $dnone ?>">
                            <i class="fa fa-copy"></i> Copy Bank Soal
                        </button>
                        <div class="btn-group <?= $dnone ?>">
                            <a href="<?= base_url('cbtbanksoal?mode=1&id=' . urlencode($id_guru)) ?>" 
                               class="btn btn-sm btn-outline-primary<?= $mode == '1' ? ' active' : '' ?>">
                                <i class="fa fa-list"></i>
                            </a>
                            <a href="<?= base_url('cbtbanksoal?mode=2&id=' . urlencode($id_guru)) ?>" 
                               class="btn btn-sm btn-outline-primary<?= $mode == '2' ? ' active' : '' ?>">
                ...
                <div class="card-body">
                    <!-- BANK SOAL DEADLINE -->
                    <div class="small-box" 
                         style="background-color: #35e1d3; background-image: linear-gradient(141deg, #9fb8ad 0%, #1fc8db 51%, #35e1d3 75%); color: white; opacity: 0.85;">
                        <div class="inner">
                            <h4>Waktu pembuatan dan penginputan bank soal akan berakhir dalam:</h4>
                            <?php
                            if ($diff <= 0 && $diff2 <= 0) {
                                echo '<h3><label class="badge bg-red">SUDAH TUTUP</label></h3>';
                            } elseif ($diff > 0) {
                                $hari = floor($jam / 24);
                                if ($hari >= 1) {
                                    $sisaJam = $jam % 24;
                                    echo "<h3>{$hari} <span class='badge badge-primary'>Hari</span> {$sisaJam} <span class='badge badge-primary'>Jam</span> {$menit} <span class='badge badge-primary'>Menit</span></h3>";
                                } elseif ($jam >= 1) {
                                    echo "<h3>{$jam} <span class='badge bg-yellow'>Jam</span> {$menit} <span class='badge bg-yellow'>Menit</span></h3>";
                                } else {
                                    echo "<h3>{$menit} <span class='badge bg-red'>Menit lagi...!</span></h3>";
                                }
                            } else {
                                echo '<h3><label class="badge bg-red">SUDAH TUTUP</label></h3>';
                            }
                            ...
                            <table class="table table-borderless table-sm">
                                <tr>
                                    <td class="align-middle text-right">
                                        <button id="submit-hapus" type="submit" class="btn btn-danger <?=$dnone?>" disabled>
                                ...
                                <table class="w-100 table table-striped table-bordered">
                                    ...
                                    foreach ($banks[$tp_active->id_tp][$smt_active->id_smt] as $bank): ?>
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle"><span class="badge badge-warning ml-2"><i class="fa fa-book mr-1"></i> <?= $bank->nama_mapel ?> </span> <br> <span class="badge badge-info ml-2"><i class="fa fa-user mr-1"></i> <?= $bank->nama_guru ?>  </span>  </td>
                                            ...
                                            <td class="text-center w-auto">
                                                <span data-toggle="tooltip" title="Edit Bank Soal">
                                                    <a type="button"
                                                       ...
                                                       class="btn btn-warning btn-sm mb-1 <?= $disable_edit ?> <?=$dnone?>"
